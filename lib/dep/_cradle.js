...
var Args = require('./vendor/vargs').Constructor;
...
cradle.Connection = function Connection(/* variable args */) {
    ...
    this.connectionSelector = 0;
    this.socket = [];
    for(var i = 0;i<this.options.connections;i++)
    {
    	this.socket.push(http.createClient(this.port, this.host));
...
//    sys.log(this.connectionSelector);
    if (!headers.Connection)
        headers.Connection = 'Keep-Alive';
    request = this.socket[ (++this.connectionSelector) % this.options.connections].request(method.toUpperCase(), path, headers);
